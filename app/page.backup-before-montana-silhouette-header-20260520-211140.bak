...
type SiteContent = {
  ...
  serviceAreaTownDescriptions: Record<string, string>;
...
const STORAGE_KEY = "stutzmans-construction-site-v14-final-montana-refinement";
const OLD_STORAGE_KEYS = ["stutzmans-construction-site-v13-service-area-clean-card", "stutzmans-construction-site-v12-montana-premium", "stutzmans-construction-site-v11", "stutzmans-construction-site-v10", "stutzmans-construction-site-v9", "stutzmans-construction-site-v8", "stutzmans-construction-site-v7", "stutzmans-construction-site-v6", "stutzmans-construction-site-v5"];
...
const defaultTownDescriptions: Record<string, string> = {
  Roosville: "A northern border-area community where durable exterior work, remodels, shops, garages, and rural property upgrades need to be built for Montana weather.",
  "West Kootenai": "A rugged mountain and lake-area community where we focus on clean craftsmanship, practical construction, and finish work that fits the land.",
  Eureka: "A core service area for custom homes, remodels, garages, shops, additions, roofing, siding, and detailed finish work.",
  Fortine: "A quiet Northwest Montana community where we help with practical builds, home improvements, exterior upgrades, and long-lasting construction.",
  Trego: "A rural work area for homes, additions, garages, shops, remodels, and exterior projects built with strength and clean detail.",
  Stryker: "A mountain-side service area where dependable construction, clean communication, and rugged finish work matter.",
  Olney: "A nearby rural community where we help with home projects, shops, remodels, garages, and exterior construction needs.",
  Yaak: "A rugged remote-area community where weather-ready construction, strong materials, and careful planning are important.",
  "nearby rural properties": "We also consider nearby rural properties around these communities. Reach out with your location and project details so we can confirm availability.",
};

function getDefaultTownDescription(town: string) {
  return defaultTownDescriptions[town] || `We serve ${town} and nearby Northwest Montana properties with rugged craftsmanship, clear communication, and clean high-end construction work.`;
}

function getTownDescription(content: SiteContent, town: string) {
  const saved = content.serviceAreaTownDescriptions?.[town]?.trim();
  return saved || getDefaultTownDescription(town);
}
...
const defaultContent: SiteContent = {
  ...
  priceText:
    "Full finished custom homes can start around $275 per square foot. Remodels, garages, shops, additions, and exterior work may be lower or higher depending on scope, finishes, materials, site access, weather, utilities, and project details.",
  ...
  integrityTitle: "Built with integrity. Built for Montana.",
...
function migrateContent(raw: unknown): SiteContent {
  ...
  return {
    ...
    serviceAreaTownDescriptions:
      parsed.serviceAreaTownDescriptions && typeof parsed.serviceAreaTownDescriptions === "object"
        ? { ...defaultTownDescriptions, ...(parsed.serviceAreaTownDescriptions as Record<string, string>) }
        : defaultTownDescriptions,
      ...
      <div className="relative z-10 pb-28 md:pb-0">
        ...
        {view === "home" && (
          <>
            ...
            <section className="mx-auto max-w-6xl px-5 py-10 md:px-7">
              ...
              <h2 className="mt-4 text-[clamp(2.25rem,6.4vw,4.7rem)] font-black tracking-[-.05em] text-[var(--title)]">Built for Montana. Finished clean.</h2>
        ...
        {view === "adminLogin" && (
          <section className="mx-auto flex min-h-[70vh] max-w-lg items-center px-5 py-16">
            <div className="w-full rounded-[2rem] border border-white/10 bg-white/8 p-7 shadow-2xl shadow-black/50 backdrop-blur-xl">
              <div className="text-xs font-black uppercase tracking-[.3em] text-[var(--label)]">Private settings</div>
              <h1 className="mt-4 text-4xl font-black tracking-[-.04em] text-[var(--title)]">Sign in</h1>
              <p className="mt-3 text-[var(--muted)]">Enter the private PIN to control website text, photos, prices, colors, fonts, project order, service area copy, and home-page project slots.</p>
        ...
        {view === "admin" && (
          <AdminPanel
            ...
            addServiceTown={addServiceTown}
            deleteServiceTown={deleteServiceTown}
...
function DecorativeMontanaLayer() {
  return (
    <div aria-hidden="true" className="pointer-events-none fixed inset-0 z-[1] overflow-hidden opacity-45">
      ...
      <div className="absolute z-[9999] bottom-24 left-0 right-0 h-28 bg-black/25">
        {[4, 11, 83, 90].map((left, index) => (
          <div
            key={left}
            className={`absolute bottom-0 bg-white/[.025] ${index % 2 === 0 ? "h-24 w-10" : "h-16 w-8"}`}
            style={{
              left: `${left}%`,
              clipPath: "polygon(50% 0, 90% 42%, 70% 42%, 100% 78%, 62% 78%, 62% 100%, 38% 100%, 38% 78%, 0 78%, 30% 42%, 10% 42%)",
            }}
          />
        ))}
      </div>
...
function ProjectsSection({ title, intro, projects, categories, activeFilter, setFilter, activePhotos, movePhoto, fullPage }: { title: string; intro: string; projects: Project[]; categories: string[]; activeFilter: string; setFilter: (filter: string) => void; activePhotos: Record<string, number>; movePhoto: (id: string, dir: 1 | -1) => void; fullPage?: boolean }) {
  return (
    <section className={`mx-auto max-w-6xl px-5 ${fullPage ? "py-10" : "py-10"} md:px-7`}>
      ...
      <h2 className="mt-4 text-[clamp(2.35rem,7vw,4.8rem)] font-black tracking-[-.05em] text-[var(--title)]">{title}</h2>
...
function AdminPanel({ content, updateContent, updateProject, setHomeSlot, addProject, deleteProject, newCategory, setNewCategory, newTown, setNewTown, addServiceTown, deleteServiceTown, addCategory, deleteCategory, uploadLogo, uploadPriceBackground, uploadProjectPhotos, reorderProjectPhoto, save, cancelChanges, savedNotice, setView }: { content: SiteContent; updateContent: (p: Partial<SiteContent>) => void; updateProject: (id: string, p: Partial<Project>) => void; setHomeSlot: (id: string, slot: 1 | 2 | 3 | null) => void; addProject: () => void; deleteProject: (id: string) => void; newCategory: string; setNewCategory: (v: string) => void; newTown: string; setNewTown: (v: string) => void; addServiceTown: () => void; deleteServiceTown: (town: string) => void; addCategory: () => void; deleteCategory: (category: string) => void; uploadLogo: (e: ChangeEvent<HTMLInputElement>) => void; uploadPriceBackground: (e: ChangeEvent<HTMLInputElement>) => void; uploadProjectPhotos: (id: string, e: ChangeEvent<HTMLInputElement>) => void; reorderProjectPhoto: (id: string, fromIndex: number, toIndex: number) => void; save: () => void | Promise<void>; cancelChanges: () => void | Promise<void>; savedNotice: string; setView: (v: View) => void }) {
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-8 md:px-7">
      <div className="sticky top-[78px] z-30 mb-6 flex flex-col gap-4 rounded-[1.6rem] border border-white/10 bg-black/60 p-4 shadow-2xl shadow-black/45 backdrop-blur-2xl md:top-[92px] md:flex-row md:items-center md:justify-between">
        <div>
          <div className="text-[10px] font-black uppercase tracking-[.32em] text-[var(--label)]">Website settings</div>
          <h1 className="mt-1 text-2xl font-black tracking-[-.04em] text-[var(--title)] md:text-3xl">Content editor</h1>
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <AdminCard title="Montana service area">
          ...
          <div className="rounded-[1.35rem] border border-white/10 bg-black/25 p-4 shadow-xl shadow-black/20">
            <div className="mb-3 text-[10px] font-black uppercase tracking-[.18em] text-[var(--label)]">Town descriptions</div>
            <p className="mb-4 text-xs font-bold leading-5 text-white/45">
              Edit the short public text that appears when each town is selected on the Where We Work section.
            </p>
            <div className="space-y-3">
              {parseServiceTowns(content.serviceAreaTownsText).map((town) => (
                <Textarea
                  key={town}
                  label={`${town} public description`}
                  value={getTownDescription(content, town)}
                  onChange={(value) =>
                    updateContent({
                      serviceAreaTownDescriptions: {
                        ...(content.serviceAreaTownDescriptions || {}),
                        [town]: value,
                      },
                    })
                  }
              ...
              {!parseServiceTowns(content.serviceAreaTownsText).length && (
                <div className="rounded-2xl border border-dashed border-white/15 p-5 text-center text-sm font-bold text-white/45">
                  Add a town first, then its description editor will appear here.
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-10 md:px-7">
      <div className="relative overflow-hidden rounded-[2.2rem] border border-white/10 bg-[#050403] p-6 shadow-2xl shadow-black/55 md:p-8 lg:p-10">
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-36 bg-white/[.025]" style={{ clipPath: "polygon(0 100%, 12% 62%, 20% 82%, 32% 46%, 45% 84%, 58% 52%, 69% 80%, 82% 58%, 100% 76%, 100% 100%)" }} />
        <div className="relative max-w-5xl">
          <div className="text-[11px] font-black uppercase tracking-[.34em] text-[var(--label)]">Where we work</div>
          <h2 className="mt-5 max-w-4xl text-[clamp(2.35rem,7vw,4.9rem)] font-black leading-[.92] tracking-[-.055em] text-[var(--title)]">
            {content.serviceAreaTitle}
          </h2>
          <p className="mt-6 max-w-4xl text-base font-bold leading-8 text-[var(--muted)] md:text-lg md:leading-9">
          ...
          <div className="mt-7 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {towns.map((town) => (
              <button
                ...
                className={`min-h-[54px] rounded-2xl border px-3 py-3 text-center text-sm font-black leading-tight transition active:scale-[.98] md:text-base ${selectedTown === town ? "border-white bg-white text-black" : "border-white/10 bg-white/8 text-white/80 hover:bg-white/12"}`}
              >
                {town}
              </button>
            ))}
          ...
          <div className="mt-7 rounded-[1.45rem] border border-white/10 bg-white/[.055] p-5 shadow-xl shadow-black/20 backdrop-blur-xl">
            ...
            <p className="mt-2 text-sm font-bold leading-6 text-white/60 md:text-base md:leading-7">
              {getTownDescription(content, selectedTown)}
            </p>
          </div>
